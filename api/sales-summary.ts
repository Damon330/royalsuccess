...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (requesterErr || requester?.status !== 'active') {
    return new Response(JSON.stringify({ error: 'Forbidden' }), { status: 403 })
  }

  let allowed = requester.role === 'admin' || agentId === user.id
  if (!allowed && requester.role === 'team_lead') {
  ...
  const cacheKey = `sales:summary:${user.id}:${agentId}:${date}`
