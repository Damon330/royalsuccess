...
3. For production/commercial use, run the migration/fix SQL files in `supabase/`, ending with `supabase/commercial-hardening.sql`. This final migration locks down role escalation, atomic sales/returns, orphaned stock repair after user deletion, payroll admin policies, and notification insert permissions.
