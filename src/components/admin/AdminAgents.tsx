      ...
      {approvingUser && (
        <RoleModal title={`Approve: ${approvingUser.full_name}`} user={approvingUser} teamLeads={teamLeads}
          ...
          onSave={async (role, tlId) => {
            const ok = await approveUser(approvingUser.id, role, tlId)
            if (ok) { refetch(); setApprovingUser(null) }
          }} />
      )}
      {editingUser && (
        <RoleModal title={`Edit: ${editingUser.full_name}`} user={editingUser} teamLeads={teamLeads}
          ...
          onSave={async (role, tlId) => {
            const ok = await updateRole(editingUser.id, role, tlId)
            if (ok) { refetch(); setEditingUser(null) }
          }} />
      )}
      {deletingUser && (
        <DeleteUserModal
