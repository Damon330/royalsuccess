...
export default function AdminReceipts() {
  ...
  async function downloadReceipt(r: Receipt) {
    if (r.pdf_url) {
      ...
      const phone = r.phone as { model?: string; imei?: string; barcode?: string } | undefined
      ...
      const pdfData: ReceiptPdfData = {
        ...
        agentId: r.agent_id.slice(0, 8).toUpperCase(),
        buyerName: r.buyer_name,
        buyerPhone: r.buyer_phone,
        phoneModel: phone?.model ?? 'Unknown',
        imei: phone?.imei ?? '',
      ...
      const blob = await pdf(createElement(ReceiptDocument as any, { data: pdfData }) as any).toBlob()
