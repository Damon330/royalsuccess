...
export function useProfiles(options: { enabled?: boolean } = {}) {
  ...
  async function approveUser(userId: string, role: Role, teamLeadId?: string) {
    try {
      ...
      if (rpcErr) {
        // RPC not deployed yet — fall back to direct update
        const isNotFound = rpcErr.code === '42883' || rpcErr.code === 'PGRST202'
        if (!isNotFound) { toast.error(`Failed: ${rpcErr.message}`); return false }

        const update: Partial<Profile> = { status: 'active', role }
        if (teamLeadId) update.team_lead_id = teamLeadId
        const { data, error } = await withTimeout(
          supabase.from('profiles').update(update).eq('id', userId).select(),
          MUTATE_TIMEOUT,
        )
        if (error) { toast.error(`Failed: ${error.message}`); return false }
        if (!data?.length) {
          toast.error('Approval blocked by database — run supabase/fix-admin-permissions.sql in Supabase SQL Editor.')
          return false
        }
      }
  ...
  async function deleteProfile(userId: string): Promise<boolean> {
    try {
      ...
      if (rpcErr) {
        ...
        const { data: childAgents, error: childErr } = await withTimeout(
          supabase.from('profiles').select('id').eq('team_lead_id', userId),
          MUTATE_TIMEOUT,
        )
        if (childErr) throw childErr

        const childIds = (childAgents ?? []).map((p) => p.id)
        if (childIds.length > 0) {
          const { error: childPhoneErr } = await withTimeout(
            supabase.from('phones')
              .update({ status: 'in_stock', assigned_to: null, assigned_at: null })
              .in('assigned_to', childIds),
            MUTATE_TIMEOUT,
          )
          if (childPhoneErr) throw childPhoneErr
        }
